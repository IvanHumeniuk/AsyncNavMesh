...
public class PathUtils
{
    ...
    public static int RetracePortalsOptimazed(NavMeshQuery query, int startIndex, int endIndex
       , NativeSlice<PolygonId> path, int n, Vector3 termPos
       , ref NativeArray<NavMeshLocation> straightPath
       , int maxStraightPath)
    {
    ...
    public static PathQueryStatus FindStraightPathOptimazed(NavMeshQuery query, Vector3 startPos, Vector3 endPos
        , NativeSlice<PolygonId> path, int pathSize
        , ref NativeArray<NavMeshLocation> straightPath
        , ref int straightPathCount
        , int maxStraightPath)
    {
        ...
        if (pathSize > 1)
        {
            ...
            for (var i = 1; i <= pathSize; ++i)
            {
                ...
                if (Perp2D(left, vr) < 0)
                {
                    ...
                    n = RetracePortalsOptimazed(query, apexIndex, leftIndex, path, n, termPos, ref straightPath, maxStraightPath);
                ...
                if (Perp2D(right, vl) > 0)
                {
                    ...
                    n = RetracePortalsOptimazed(query, apexIndex, rightIndex, path, n, termPos, ref straightPath, maxStraightPath);
        ...
        if (n >= 1)
            n = RetracePortalsOptimazed(query, apexIndex, pathSize - 1, path, n, endPos, ref straightPath, maxStraightPath);
